...
function toMarkdown(data, opts = {}) {
  ...
  L.push('> 📋 이 요약은 규칙별 **대표 위치 1건**만 보여줍니다. 고칠 수 있는 **전체 위반 목록**(모든 파일·라인·조치)은 CSV로 받으세요 — `scan_report`의 `format:"csv"` 또는 `report-cli --format csv`.', '');
...
// CSV 셀 안전화(따옴표 이스케이프 + 전체를 큰따옴표로 감쌈 — 콤마/개행/따옴표 안전).
function csvCell(s) { return '"' + String(s == null ? '' : s).replace(/"/g, '""') + '"'; }

/**
 * scanResult → 전체 위반을 한 줄씩 펼친 CSV (요약이 아니라 "작업용 전체 목록").
 * 스프레드시트에서 도메인·심각도·파일별로 정렬/필터/체크하며 고칠 수 있다.
 * (요약 리포트는 규칙당 샘플 3개만 보여주므로, 전체 목록은 이 함수를 쓴다.)
 */
function toCsv(scanResult) {
  const header = ['domain', 'ruleId', 'title', 'severity', 'tier', 'autoFixable', 'file', 'line', 'code', 'suggestion'];
  const lines = [header.map(csvCell).join(',')];
  for (const d of (scanResult && scanResult.results) || []) {
    for (const v of (d.violations) || []) {
      lines.push([
        d.domain, v.id, v.title, v.severity, v.tier, v.autoFixable,
        shortFile(v.file), v.line,
        (v.code || '').replace(/\s+/g, ' ').trim().slice(0, 300),
        v.suggestion || ''
      ].map(csvCell).join(','));
    }
  }
...
/** 통합 진입점: scanResult → { data, markdown, html, csv }. ruleIndex 미주입 시 내장 rules/ 로드. */
function generateReport(scanResult, opts = {}) {
  ...
  return { data, markdown: toMarkdown(data, opts), html: toHtml(data, opts), csv: toCsv(scanResult) };
}

module.exports = { generateReport, summarize, toMarkdown, toHtml, toCsv, mdCell, DOMAIN_META };
